...
import { EmployeeTypeHook } from "../../../hooks/globalMasters/employeeTypeHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteEmployeeTypeMaster,
...
const EmployeeTypeList: FC = () => {
  const { data: employeetypes, setData: setUser } = EmployeeTypeHook(true);
  ...
  const getDeleteEmployeeTypeData = useSelector(getDeleteEmployeeTypeMaster);
  ...
  const navigateToAddEmployeeType = () => {
    navigate("/employee-type/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/employee-type/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/employee-type/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/employee-type/add")
          : setOpenDialog(true);
  };
  ...
  const deleteEmployeeTypeClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteEmployeeTypeMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        <MastersHeader
          ...
          BreadcrumSubTitle={"Employee Type List"}
        />
      </div>
      <Paper elevation={3}>
        <Box p={2} pt={0}>
                  ...
                  <Controller
                    render={({ field: any }) => (
                      <TextField
                        ...
                        label="Search"
                        id="inputName"
                        placeholder="Search EmployeeType"
                        value={search}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Button
                        ...
                        variant="contained"
                        onClick={navigateToAddEmployeeType}
                        startIcon={<AddIcon />}
                      >
                        ADD NEW
                      </Button>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteEmployeeTypeClick}
