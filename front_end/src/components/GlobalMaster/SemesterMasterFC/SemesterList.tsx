...
import { SemesterHook } from "../../../hooks/globalMasters/semesterHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import { actions, getDeleteSemesterMaster } from "../../../store/GlobalMaster/semesterMaster";
...
const SemesterList: FC = () => {
  const { data: semesters, setData: setUser } = SemesterHook(true);
  ...
  const getDeleteSemesterData = useSelector(getDeleteSemesterMaster);
  ...
  const navigateToAddSemester = () => {
    navigate("/semester/add");
  };
  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/semester/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/semester/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/semester/add")
          : setOpenDialog(true);
  };
  ...
  const deleteSemesterClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteSemesterMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Semester List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteSemesterClick}
