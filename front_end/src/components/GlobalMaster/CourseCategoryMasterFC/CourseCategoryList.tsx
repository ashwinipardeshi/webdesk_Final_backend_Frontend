...
import { CourseCategoryHook } from "../../../hooks/globalMasters/courseCategoryHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteCourseCategoryMaster
...
const CourseCategoryList: FC = () => {
  const { data: courseCategories, setData: setUser } = CourseCategoryHook(true);
  ...
  const getDeleteCourseCategoryData = useSelector(getDeleteCourseCategoryMaster);
  ...
  const navigateToAddCourseCategory = () => {
    navigate("/course-category/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/course-category/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/course-category/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/course-category/add")
          : setOpenDialog(true);
  };
  ...
  const deleteCourseCategoryClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteCourseCategoryMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Course Category List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Button
                        ...
                        variant="contained"
                        onClick={navigateToAddCourseCategory}
                        startIcon={<AddIcon />}
                      >
                        ADD NEW
                      </Button>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteCourseCategoryClick}
