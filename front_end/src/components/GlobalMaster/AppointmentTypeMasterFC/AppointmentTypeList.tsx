...
import { AppointmentTypeHook } from "../../../hooks/globalMasters/appointmentTypeHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteAppointmentTypeMaster,
...
const AppointmentTypeList: FC = () => {
  const { data: appointmentTypes, setData: setUser } = AppointmentTypeHook(true);
  ...
  const getDeleteAppointmentTypeData = useSelector(getDeleteAppointmentTypeMaster);
  ...
  const navigateToAddAppointmentType = () => {
    navigate("/appointment-type/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/appointment-type/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/appointment-type/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/appointment-type/add")
          : setOpenDialog(true);
  };
  ...
  const deleteAppointmentTypeClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteAppointmentTypeMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
  ...
  return (
    <>
      ...
      <div>
        {/* <!-- breadcrumb --> */}
        <MastersHeader
          title={"AppointmentType List"}
          BreadcrumbTitle={"Appointment Type"}
          BreadcrumSubTitle={"Appointment Type List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
                  ...
                  <Controller
                    render={({ field: any }) => (
                      <TextField
                        ...
                        label="Search"
                        id="inputName"
                        placeholder="Search AppointmentType"
                        value={search}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Button
                        ...
                        variant="contained"
                        onClick={navigateToAddAppointmentType}
                        startIcon={<AddIcon />}
                      >
                        ADD NEW
                      </Button>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteAppointmentTypeClick}
        rowParam={selectedRowData}
        page={"appointment-Type"}
      />
    </>
  );
};
