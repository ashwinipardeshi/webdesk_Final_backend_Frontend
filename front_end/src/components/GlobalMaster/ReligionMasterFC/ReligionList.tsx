...
import { ReligionHook } from "../../../hooks/globalMasters/religionHook";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteReligionMaster
...
const ReligionList: FC = () => {
  const { data: religions, setData: setUser } = ReligionHook(true);
  ...
  const getDeleteReligionData = useSelector(getDeleteReligionMaster);
  ...
  const navigateToAddReligion = () => {
    navigate("/religion/add");
  };
  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/religion/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/religion/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/religion/add")
          : setOpenDialog(true);
  };
  ...
  const deleteReligionClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteReligionMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Religion List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteReligionClick}
