...
import { MinorityHook } from "../../../hooks/globalMasters/minorityHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import {
  actions,
  getDeleteMinorityMaster,
...
const MinorityList: FC = () => {
  const { data: minorities, setData: setUser } = MinorityHook(true);
  ...
  const getDeleteMinorityData = useSelector(getDeleteMinorityMaster);
  ...
  const navigateToAddMinority = () => {
    navigate("/minority/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/minority/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/minority/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/minority/add")
          : setOpenDialog(true);
  };
  ...
  const deleteMinorityClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteMinorityMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Minority List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteMinorityClick}
