...
import { DomicileHook } from "../../../hooks/globalMasters/domicileHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteDomicileMaster,
...
const DomicileList: FC = () => {
  const { data: domicile, setData: setUser } = DomicileHook(true);
  ...
  const getDeleteDomicileData = useSelector(getDeleteDomicileMaster);
  ...
  const navigateToAddDomicile = () => {
    navigate("/domicile/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/domicile/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/domicile/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/domicile/add")
          : setOpenDialog(true);
  };
  ...
  const deleteDomicileClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteDomicileMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Domicile List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteDomicileClick}
