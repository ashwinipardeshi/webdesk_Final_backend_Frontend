...
import { TimeSlotHook } from "../../../hooks/globalMasters/timeSlotHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteTimeSlotMaster,
...
const TimeSlotList: FC = () => {
  const { data: timeSlotData, setData: setUser } = TimeSlotHook(true);
  ...
  const getDeleteTimeSlotData = useSelector(getDeleteTimeSlotMaster);
  ...
  const navigateToAddTimeSlot = () => {
    navigate("/time-slot/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/time-slot/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/time-slot/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/time-slot/add")
          : setOpenDialog(true);
  };
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      renderCell: (rowData: any) =>
        rowData.value ? moment(rowData.value).utc(true).local().format("hh:mm A") : "",
    },
    {
      field: "toTime",
      headerName: "To Time",
  ...
  const deleteCountyClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteTimeSlotMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Time Slot List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteCountyClick}
