...
import { BloodGroupHook } from "../../../hooks/globalMasters/bloodGroupHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteBloodGroupMaster,
...
const BloodGroupList: FC = () => {
    const { data: bloodgroups, setData: setUser } = BloodGroupHook(true);
    ...
    const getDeleteBloodGroupData = useSelector(getDeleteBloodGroupMaster);
    ...
    const navigateToAddBloodGroup = () => {
        navigate("/blood-group/add");
    };

    const handleActionClick = (event: any, rowData: any, type: string) => {
        setSelectedRowData(rowData);
        type === "View"
            ? navigate(`/blood-group/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/blood-group/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/blood-group/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteBloodGroupClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteBloodGroupMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
        ...
        <>
            ...
            <div>
                <MastersHeader
                    ...
                    BreadcrumSubTitle={"Blood Group List"}
                />
            </div>
            <Paper elevation={3}>
                <Box p={2} pt={0}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddBloodGroup}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteBloodGroupClick}
