...
import { CasteHook } from "../../../hooks/globalMasters/casteHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import {
  actions,
  getDeleteCasteMaster,
...
const CasteList: FC = () => {
  const { data: castes, setData: setUser } = CasteHook(true);
  ...
  const getDeleteCasteData = useSelector(getDeleteCasteMaster);
  ...
  const handleActionClick = (event: any, rowData: any, type: string) => {
    ...
    type === "View"
      ? navigate(`/caste/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/caste/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/caste/add")
          : setOpenDialog(true);
  };
  ...
  const deleteCasteClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteCasteMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
  ...
  return (
    <>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteCasteClick}
