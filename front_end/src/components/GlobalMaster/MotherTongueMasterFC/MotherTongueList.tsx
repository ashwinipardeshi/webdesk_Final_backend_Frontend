...
import { MotherTongueHook } from "../../../hooks/globalMasters/motherTongueHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteMotherTongueMaster,
...
const MotherTongueList: FC = () => {
    const { data: mothertongues, setData: setUser } = MotherTongueHook(true);
    ...
    const getDeleteMotherTongueData = useSelector(getDeleteMotherTongueMaster);
    ...
    const navigateToAddMotherTongue = () => {
        navigate("/mother-tongue/add");
    };

    const handleActionClick = (event: any, rowData: any, type: string) => {
        setSelectedRowData(rowData);
        type === "View"
            ? navigate(`/mother-tongue/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/mother-tongue/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/mother-tongue/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteMontherTongueClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteMotherTongueMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
    ...
    return (
        <>
            ...
            <div>
                <MastersHeader
                    title={"Mother Tongue List"}
                    BreadcrumbTitle={"Mother Tongue"}
                    BreadcrumSubTitle={"Mother Tongue List"}
                />
            </div>
            <Paper elevation={3}>
                <Box p={2} pt={0}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddMotherTongue}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteMontherTongueClick}
...
export default MotherTongueList;
