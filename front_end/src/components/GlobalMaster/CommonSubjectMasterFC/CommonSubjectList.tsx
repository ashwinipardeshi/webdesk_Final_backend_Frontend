...
import { CommonSubjectHook } from "../../../hooks/globalMasters/commonSubjectHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteCommonSubjectMaster,
...
const CommonSubjectList: FC = () => {
    const { data: commonsubjectes, setData: setUser } = CommonSubjectHook(true);

    interface IFormInput {
        commonSubjectName: string;
    }
    ...
    const getDeleteCommonSubjectData = useSelector(getDeleteCommonSubjectMaster);
    ...
    const navigateToAddCommonSubject = () => {
        navigate("/common-subject/add");
    };

    const handleActionClick = (event: any, rowData: any, type: string) => {
        setSelectedRowData(rowData);
        type === "View"
            ? navigate(`/common-subject/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/common-subject/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/common-subject/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteCommonClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteCommonSubjectMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
        ...
        <>
            ...
            <div>
                <MastersHeader
                    ...
                    BreadcrumSubTitle={"Common Subject List"}
                />
            </div>
            <Paper elevation={3}>
                <Box p={2} pt={0}>
                                    ...
                                    <Controller
                                        render={({ field: any }) => (
                                            <TextField
                                                ...
                                                label="Search"
                                                id="inputName"
                                                placeholder="Search Common Subject"
                                                value={search}
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddCommonSubject}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteCommonClick}
