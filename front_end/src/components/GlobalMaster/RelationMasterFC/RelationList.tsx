...
import { RelationHook } from "../../../hooks/globalMasters/relationHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteRelationMaster,
...
const RelationList: FC = () => {
  const { data: relations, setData: setUser } = RelationHook(true);
  ...
  const getDeleteRelationData = useSelector(getDeleteRelationMaster);
  ...
  const handleActionClick = (event: any, rowData: any, type: string) => {
    ...
    type === "View"
      ? navigate(`/relation/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/relation/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/relation/add")
          : setOpenDialog(true);
  };
  ...
  const deleteRelationClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteRelationMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        <MastersHeader
          ...
          BreadcrumSubTitle={"Relation List"}
        />
      </div>
      <Paper elevation={3}>
        <Box p={2} pt={0}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteRelationClick}
