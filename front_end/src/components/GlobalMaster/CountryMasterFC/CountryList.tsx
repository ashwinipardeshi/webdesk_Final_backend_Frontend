...
import { CountryHook } from "../../../hooks/globalMasters/countryHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteCountryMaster,
...
const CountryList: FC = () => {
  const { data: countries, setData: setUser } = CountryHook(true);
  ...
  const getDeleteCountryData = useSelector(getDeleteCountryMaster);
  ...
  const handleActionClick = (event: any, rowData: any, type: string) => {
    ...
    type === "View"
      ? navigate(`/country/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/country/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/country/add")
          : setOpenDialog(true);
  };
  ...
  const deleteCountyClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteCountryMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Country List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
