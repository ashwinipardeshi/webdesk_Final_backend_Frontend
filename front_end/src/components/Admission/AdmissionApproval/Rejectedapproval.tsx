...
const RejectedapprovalForm: FC = (props: any) => {
    ...
    interface Column {
        field: "ApplicationNo" | "Student Name" | "MobileNo" | "BranchName" | "ProgramYearName" | "CreatedDate" | "ApplicationFor" | "ApplicationRejectReasonName" | "action";
        headerName: string;
        minWidth?: number;
        align?: "right";
        flex?: number;
    ...
    const columns: Column[] = [
        ...
        {
            field: "ApplicationRejectReasonName",
    ...
    const filteredData = useMemo(() => {
        if (requestParams && requestParams.academicyearid !== "" && requestParams.programid === "" && requestParams.programyearid === "" && requestParams.branchid === "" && requestParams.fromdate === "" && requestParams.todate === "") {
            return onlineapprovals.filter(
                (x: any) => x.AdmissionStatus === "REJECT" && x.AcademicYearId === Number(requestParams.academicyearid)
            );
            ...
            return onlineapprovals.filter(
                (x: any) => x.AdmissionStatus === "REJECT" && x.AcademicYearId === Number(requestParams.academicyearid) && x.BranchId === Number(requestParams.branchid)
            );

        } else if (requestParams && requestParams.academicyearid !== "" && requestParams.programid === "" && requestParams.programyearid !== "" && requestParams.branchid === "" && requestParams.fromdate === "" && requestParams.todate === "") {

            return onlineapprovals.filter(
                (x: any) => x.AdmissionStatus === "REJECT" && x.AcademicYearId === Number(requestParams.academicyearid) && x.ProgramYearId === Number(requestParams.programyearid)
            );
        } else if (requestParams && requestParams.academicyearid !== "" && requestParams.programid !== "" && requestParams.programyearid !== "" && requestParams.branchid !== "" && requestParams.fromdate === "" && requestParams.todate === "") {

            return onlineapprovals.filter(
                (x: any) => x.AdmissionStatus === "REJECT" && x.AcademicYearId === Number(requestParams.academicyearid) && x.BranchId === Number(requestParams.branchid) && x.ProgramYearId === Number(requestParams.programyearid)
            );
            ...
            return onlineapprovals.filter(
                (x: any) => x.AdmissionStatus === "REJECT" && x.AcademicYearId === Number(requestParams.academicyearid) && x.BranchId === Number(requestParams.branchid) && x.ProgramYearId === Number(requestParams.programyearid) && x.CreatedDate >= requestParams.fromdate && x.CreatedDate <= requestParams.todate
            );
        } else {
            return onlineapprovals.filter(
