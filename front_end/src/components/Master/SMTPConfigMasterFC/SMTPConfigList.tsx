...
import { SMTPConfigHook } from "../../../hooks/masters/smtpConfigHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteSMTPConfigMaster,
...
const SMTPConfigList: FC = () => {
  const { data: SMTPConfigs, setData: setUser } = SMTPConfigHook(true);
  ...
  const getDeleteSMTPConfigData = useSelector(getDeleteSMTPConfigMaster);
  ...
  const navigateToAddSMTPConfig = () => {
    navigate("/smtp-config/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/smtp-config/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/smtp-config/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/smtp-config/add")
          : setOpenDialog(true);
  };
  ...
  const deleteSMTPConfigClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteSMTPConfigMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
  ...
  return (
    <>
      ...
      <div>
        {/* <!-- breadcrumb --> */}
        <MastersHeader
          title={"SMTP Config List"}
          BreadcrumbTitle={"SMTP Config"}
          BreadcrumSubTitle={"SMTP Config List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
                  ...
                  <Controller
                    render={({ field: any }) => (
                      <TextField
                        ...
                        label="Search"
                        id="inputName"
                        placeholder="Search SMTP Config"
                        value={search}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Button
                        ...
                        variant="contained"
                        onClick={navigateToAddSMTPConfig}
                        startIcon={<AddIcon />}
                      >
                        ADD NEW
                      </Button>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteSMTPConfigClick}
