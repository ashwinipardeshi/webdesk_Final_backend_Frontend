...
import { ProgramYearHook } from "../../../hooks/masters/programYearHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteProgramYearMaster,
...
const ProgramYearList: FC = () => {
    const { programyeardata: programyeardatas, setProgramYearData: setUser } = ProgramYearHook(true);
    ...
    const getDeleteProgramYearData = useSelector(getDeleteProgramYearMaster);
    ...
    const navigateToAddProgramYear = () => {
        navigate("/program-year/add");
    };

    const handleActionClick = (event: any, rowData: any, type: string) => {
        setSelectedRowData(rowData);
        type === "View"
            ? navigate(`/program-year/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/program-year/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/program-year/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteProgramYearClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteProgramYearMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
    ...
    return (
        <>
            ...
            <div>
                {/* <!-- breadcrumb --> */}
                <MastersHeader
                    title={"Program Year List"}
                    BreadcrumbTitle={"Program Year"}
                    BreadcrumSubTitle={"Program Year List"}
                />
                {/* <!-- /breadcrumb --> */}
            </div>
            <Paper elevation={3}>
                                    ...
                                    <Controller
                                        render={({ field: any }) => (
                                            <TextField
                                                ...
                                                label="Search"
                                                id="inputName"
                                                placeholder="Search Program Year"
                                                value={search}
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddProgramYear}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteProgramYearClick}
...
export default ProgramYearList;
