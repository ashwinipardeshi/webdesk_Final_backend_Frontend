...
import { AcademicYearHook } from "../../../hooks/masters/academicYearHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteAcademicYearMaster,
...
const AcademicYearList: FC = () => {
    const { data: AcademicYears, setData: setUser } = AcademicYearHook(true);
    ...
    const getDeleteAcademicYearData = useSelector(getDeleteAcademicYearMaster);
    ...
    const navigateToAddAcademicYear = () => {
        navigate("/academic-year/add");
    };

    const handleActionClick = (event: any, rowData: any, type: string) => {
        setSelectedRowData(rowData);
        type === "View"
            ? navigate(`/academic-year/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/academic-year/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/academic-year/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteAcademicYearClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteAcademicYearMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
        ...
        <>
            ...
            <div>
                ...
                <MastersHeader
                    ...
                    BreadcrumSubTitle={"Academic Year List"}
                />
                {/* <!-- /breadcrumb --> */}
            </div>
            <Paper elevation={3}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddAcademicYear}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteAcademicYearClick}
