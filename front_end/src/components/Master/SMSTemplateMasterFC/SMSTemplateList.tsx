...
import { SMSTemplateHook } from "../../../hooks/masters/smsTemplateHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import { 
    actions,
    getDeleteSMSTemplateMaster,
...
const SMSTemplateList: FC = () => {
    const { data: SMSTemplates, setData: setUser } = SMSTemplateHook(true);
    ...
    const getDeleteSMSTemplateData = useSelector(getDeleteSMSTemplateMaster);
    ...
    const navigateToAddSMSTemplate = () => {
    ...
    const handleActionClick = (event: any, rowData: any, type: string) => {
        ...
        type === "View"
            ? navigate(`/sms-template/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/sms-template/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/sms-template/add")
                    : setOpenDialog(true);
    };
    interface Column {
        field: "name" | "template" | "isActive" | "action";
        headerName: string;
        minWidth?: number;
        align?: "right";
        flex?: number;
    ...
    const deleteSMSTemplateClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteSMSTemplateMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
        ...
        <>
            ...
            <div>
                <MastersHeader
                    ...
                    BreadcrumSubTitle={"SMS Template List"}
                />
            </div>
            <Paper elevation={3}>
                <Box p={2} pt={0}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => (
                                            <Button
                                                ...
                                                variant="contained"
                                                onClick={navigateToAddSMSTemplate}
                                                startIcon={<AddIcon />}
                                            >
                                                ADD NEW
                                            </Button>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteSMSTemplateClick}
