...
import { DocumentHook } from '../../../hooks/masters/documentMasterHook';
...
import { getDeleteDocumentMaster } from '../../../store/Master/documentMaster';
...
const DocumentMasterList = () => {
  const {data:documentList} = DocumentHook(true)
...
const getDeleteDocumentData = useSelector(getDeleteDocumentMaster);
...
const handleActionClick = (event: any, rowData: any, type: string) => {
  ...
  type === "View"
      ? navigate(`/document/view/${rowData.id}`)
      : type === "Edit"
          ? navigate(`/document/update/${rowData.id}`)
          : type === "Add"
              ? navigate("/document/add")
              : setOpenDialog(true);
};
...
const deleteDocumentClick = () => {
  if (selectedRowData) {
      dispatch(actions.loadDeleteDocumentMaster(selectedRowData));
      setOpenDialog(false);
  }
};
  ...
  return (
    ...
    <div>
        ...
        <MastersHeader
            ...
            BreadcrumSubTitle={"Document List"}
        />
        {/* <!-- /breadcrumb --> */}
    </div>
    <Paper elevation={3}>
    ...
    <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteDocumentClick}
