...
import { FeeHeadHook } from "../../../hooks/masters/feeheadHooks";
...
const FeeHeadList: FC = () => {
  const [selectedFeeHeadType, setSelectedFeeHeadType] = useState("");
  const { feeheaddata: feeheaddatas, setFeeHeadData: setUser } = FeeHeadHook(
    true,
    Number(selectedFeeHeadType)
  );
  ...
  const navigateToAddFeeHead = () => {
    navigate("/fee-head/add");
  };

  const value = watch("feeHeadTypeMasterId");

  useEffect(() => {
    setSelectedFeeHeadType(value);
  }, [value]);

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/fee-head/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/fee-head/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/fee-head/add")
          : setOpenDialog(true);
  };
  ...
  const deleteFeeHeadClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteFeeHeadMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
    ...
    <>
      ...
      <div>
        ...
        <MastersHeader
          ...
          BreadcrumSubTitle={"Fee Head List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteFeeHeadClick}
