...
import {
  actions,
  getDeleteReservationCategoryMaster,
...
const ReservationCategoryList: FC = () => {
  const { data: ReservationCategories, setData: setUser } = ReservationCategoryHook(true);
  ...
  const getDeleteReservationCategoryData = useSelector(getDeleteReservationCategoryMaster);
  ...
  const navigateToAddReservationCategory = () => {
    navigate("/reservation-category/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/reservation-category/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/reservation-category/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/reservation-category/add")
          : setOpenDialog(true);
  };
  ...
  const deleteReservationCategorylick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteReservationCategoryMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
  ...
  const filteredData = useMemo(() => {
    return ReservationCategories.filter(
      (x: any) => !search || x.name.toLowerCase().includes(search.toLowerCase())
    );
  }, [ReservationCategories, search]);
  return (
    <>
      ...
      <div>
        {/* <!-- breadcrumb --> */}
        <MastersHeader
          title={"Reservation Category List"}
          BreadcrumbTitle={"Reservation Category"}
          BreadcrumSubTitle={"Reservation Category List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
                  ...
                  <Controller
                    render={({ field: any }) => (
                      <TextField
                        ...
                        label="Search"
                        id="inputName"
                        placeholder="Search Reservation Category"
                        value={search}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Button
                        ...
                        variant="contained"
                        onClick={navigateToAddReservationCategory}
                        startIcon={<AddIcon />}
                      >
                        ADD NEW
                      </Button>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteReservationCategorylick}
        rowParam={selectedRowData}
        page={"ReservationCategory"}
      />
    </>
  );
};
