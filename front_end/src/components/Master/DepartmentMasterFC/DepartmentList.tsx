...
import { DepartmentHook } from "../../../hooks/masters/departmentHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
    actions,
    getDeleteDepartmentMaster,
...
const DepartmentList: FC = () => {
    const { data: Departments, setData: setUser } = DepartmentHook(true);
    ...
    const getDeleteDepartmentData = useSelector(getDeleteDepartmentMaster);
    ...
    const handleActionClick = (event: any, rowData: any, type: string) => {
        ...
        type === "View"
            ? navigate(`/department/view/${rowData.id}`)
            : type === "Edit"
                ? navigate(`/department/update/${rowData.id}`)
                : type === "Add"
                    ? navigate("/department/add")
                    : setOpenDialog(true);
    };
    ...
    const deleteDepartmentClick = () => {
        if (selectedRowData) {
            dispatch(actions.loadDeleteDepartmentMaster(selectedRowData));
            setOpenDialog(false);
        }
    };
        ...
        <>
            ...
            <div>
                ...
                <MastersHeader
                    ...
                    BreadcrumSubTitle={"Department List"}
                />
                {/* <!-- /breadcrumb --> */}
            </div>
            <Paper elevation={3}>
            ...
            <DeleteConfirmation
                ...
                actionToggleDeleteDialog={deleteDepartmentClick}
