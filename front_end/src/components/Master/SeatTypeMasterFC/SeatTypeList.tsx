...
import { SeatTypeHook } from "../../../hooks/masters/seatTypeHooks";
import DeleteConfirmation from "../../../layouts/deleteConfirmation";
import { selectStatus } from "../../../store/loader";
import Loader from "../../../shade/Loaders/Loaders";
import {
  actions,
  getDeleteSeatTypeMaster,
...
const SeatTypeList: FC = () => {
  const { data: SeatTypes, setData: setUser } = SeatTypeHook(true);
  ...
  const getDeleteSeatTypeData = useSelector(getDeleteSeatTypeMaster);
  ...
  const navigateToAddSeatType = () => {
    navigate("/seat-type/add");
  };

  const handleActionClick = (event: any, rowData: any, type: string) => {
    setSelectedRowData(rowData);
    type === "View"
      ? navigate(`/seat-type/view/${rowData.id}`)
      : type === "Edit"
        ? navigate(`/seat-type/update/${rowData.id}`)
        : type === "Add"
          ? navigate("/seat-type/add")
          : setOpenDialog(true);
  };
  ...
  const deleteSeatTypeClick = () => {
    if (selectedRowData) {
      dispatch(actions.loadDeleteSeatTypeMaster(selectedRowData));
      setOpenDialog(false);
    }
  };
  ...
  return (
    <>
      ...
      <div>
        {/* <!-- breadcrumb --> */}
        <MastersHeader
          title={"Seat Type List"}
          BreadcrumbTitle={"Seat Type"}
          BreadcrumSubTitle={"Seat Type List"}
        />
        {/* <!-- /breadcrumb --> */}
      </div>
      <Paper elevation={3}>
      ...
      <DeleteConfirmation
        ...
        actionToggleDeleteDialog={deleteSeatTypeClick}
