...
namespace SaaSAppAPI.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class CollegeModuleController : ControllerBase
    {
        private readonly ICollegeModuleService _service;
        public CollegeModuleController(ICollegeModuleService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Insert(CollegeModuleVM collegeModuleVM)
        {
            if (collegeModuleVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(collegeModuleVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CollegeModuleVM collegeModuleVM)
        {
            if (id != collegeModuleVM.Id || collegeModuleVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(collegeModuleVM);
