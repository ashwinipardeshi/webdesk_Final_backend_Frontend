...
namespace SaaSAppAPI.Controllers
{
   ...
   [Authorize]
    public class PricingController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Insert(PricingVM pricingVM)
        {
            if (pricingVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(pricingVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, PricingVM pricingVM)
        {
            if (id != pricingVM.Id || pricingVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(pricingVM);
