...
namespace SaaSAppAPI.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class SaasUserMasterController : ControllerBase
    {
        private readonly IUserMasterService _service;
        public SaasUserMasterController(IUserMasterService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Insert(UserMasterVM userMasterVM)
        {
            if (userMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(userMasterVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, UserMasterVM userMasterVM)
        {
            if (id != userMasterVM.Id || userMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(userMasterVM);
