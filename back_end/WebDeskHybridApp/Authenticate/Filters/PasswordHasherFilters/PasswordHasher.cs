...
namespace Authenticate.Filters.PasswordHasherFilters
{
    /// <summary>
    ...
    public class PasswordHasher<TUser> : IPasswordHasher<TUser> where TUser : class
    {
...
#else
            return false; //#error Update target frameworks
#endif
        }
        ...
        public virtual PasswordVerificationResult VerifyHashedPasswordv4(OnlineSignInVM onlineSignInVM, string hashedPassword, string providedPassword)
        {
            if (hashedPassword == null)
            {
                throw new ArgumentNullException(nameof(hashedPassword));
