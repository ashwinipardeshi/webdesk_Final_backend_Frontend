...
namespace MasterWebAPI.BackgroundServices
{
    public class RabbitMQBackgroundConsumerService : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += (ch, ea) =>
            {
                ...
                var countryGmaster = JsonConvert.DeserializeObject<CountryGmaster>(message);

                //Stored Offer Details into the Database
                using (var scope = serviceScopeFactory.CreateScope())
                {
                    var _dbContext = scope.ServiceProvider.GetRequiredService<MasterDevFinalDbContext>();
                    var result = _dbContext.CountryGmasters.Add(countryGmaster);
