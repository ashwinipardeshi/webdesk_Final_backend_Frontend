...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ReservationCategoryMasterController : ControllerBase
    {
        private readonly IReservationCategoryService _service;
        public ReservationCategoryMasterController(IReservationCategoryService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(ReservationCategoryMasterVM reservationCategoryMasterVM)
        {
            if (reservationCategoryMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(reservationCategoryMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ReservationCategoryMasterVM reservationCategoryMasterVM)
        {
            if (id != reservationCategoryMasterVM.Id || reservationCategoryMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(reservationCategoryMasterVM);
