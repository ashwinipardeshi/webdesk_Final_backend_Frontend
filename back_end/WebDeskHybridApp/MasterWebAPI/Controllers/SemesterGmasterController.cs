...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SemesterGMasterController : ControllerBase
    {  
        private readonly ISemesterService _service;
        public SemesterGMasterController(ISemesterService service)
        {   
            _service = service;
        }
        ...
        [HttpPost] 
        public async Task<IActionResult> Insert(SemesterGMasterVM semesterMasterVM)
        {
            if (semesterMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(semesterMasterVM);
        ...
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, SemesterGMasterVM semesterMasterVM)
        {
            if (id != semesterMasterVM.Id || semesterMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(semesterMasterVM);
