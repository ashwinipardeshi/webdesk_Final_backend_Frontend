...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ApplicationRejectReasonsController : ControllerBase
    {
        private readonly IApplicationRejectReasonsService _service;
        public ApplicationRejectReasonsController(IApplicationRejectReasonsService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>    
        [HttpPost]
        public async Task<IActionResult> Insert(ApplicationRejectReasonsMasterVM applicationRejectReasonsMasterVM)
        {
            if (applicationRejectReasonsMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(applicationRejectReasonsMasterVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ApplicationRejectReasonsMasterVM applicationRejectReasonsMasterVM)
        {
            if (id != applicationRejectReasonsMasterVM.Id || applicationRejectReasonsMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(applicationRejectReasonsMasterVM);
