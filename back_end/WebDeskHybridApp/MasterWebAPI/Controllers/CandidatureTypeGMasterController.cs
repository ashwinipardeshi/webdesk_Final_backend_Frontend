...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CandidatureTypeGMasterController : ControllerBase
    {
        private readonly ICandidatureTypeService _service;
        public CandidatureTypeGMasterController(ICandidatureTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(CandidatureTypeGMasterVM candidatureTypeGMasterVM)
        {
            if (candidatureTypeGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(candidatureTypeGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CandidatureTypeGMasterVM candidatureTypeGMasterVM)
        {
            if (id != candidatureTypeGMasterVM.Id || candidatureTypeGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(candidatureTypeGMasterVM);
