...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class AdmissionTypeMasterController : ControllerBase
    {
        private readonly IAdmissionTypeService _service;
        public AdmissionTypeMasterController(IAdmissionTypeService service)
        {
            _service = service;
        }
        ...
        /// Insert
        /// </summary>
        /// <param name="amissionTypeMasterVM"></param>
        /// <returns></returns>       
     
        [HttpPost]
        public async Task<IActionResult> Insert(AdmissionTypeMasterVM amissionTypeMasterVM)
        {
            if (amissionTypeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(amissionTypeMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, AdmissionTypeMasterVM amissionTypeMasterVM)
        {
            if (id != amissionTypeMasterVM.Id || amissionTypeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(amissionTypeMasterVM);
