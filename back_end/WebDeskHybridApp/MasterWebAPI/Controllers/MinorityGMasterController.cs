...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class MinorityGMasterController : ControllerBase
    {
        private readonly IMinorityService _service;
        public MinorityGMasterController(IMinorityService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(MinorityGMasterVM minorityGMasterVM)
        {
            if (minorityGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(minorityGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, MinorityGMasterVM minorityGMasterVM)
        {
            if (id != minorityGMasterVM.Id || minorityGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(minorityGMasterVM);
