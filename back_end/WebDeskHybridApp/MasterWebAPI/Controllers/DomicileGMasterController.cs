...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class DomicileGMasterController : ControllerBase
    {
        private readonly IDomicileService _service;
        public DomicileGMasterController(IDomicileService service)
        {
            _service = service;
        }
        ...
        [HttpPost]  
        public async Task<IActionResult> Insert(DomicileGMasterVM domicileGmasterVM)
        {
            if (domicileGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(domicileGmasterVM);
        ...
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, DomicileGMasterVM domicileGmasterVM)
        {
            if (id != domicileGmasterVM.Id || domicileGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(domicileGmasterVM);
