...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ApplicationStatusMasterController : ControllerBase
    {
        private readonly IApplicationStatusService _service;
        public ApplicationStatusMasterController(IApplicationStatusService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(ApplicationStatusMasterVM applicationStatusMasterVM)
        {
            if (applicationStatusMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(applicationStatusMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ApplicationStatusMasterVM applicationStatusMasterVM)
        {
            if (id != applicationStatusMasterVM.Id || applicationStatusMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(applicationStatusMasterVM);
