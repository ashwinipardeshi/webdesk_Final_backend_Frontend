...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class MotherTongueGMasterController : ControllerBase
    {
        private readonly IMotherTongueService _service;
        public MotherTongueGMasterController(IMotherTongueService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(MotherTongueGMasterVM motherTongueGMasterVM)
        {
            if (motherTongueGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(motherTongueGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, MotherTongueGMasterVM motherTongueGMasterVM)
        {
            if (id != motherTongueGMasterVM.Id || motherTongueGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(motherTongueGMasterVM);
