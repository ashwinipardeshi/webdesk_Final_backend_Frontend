...
namespace MasterWebAPI.Controllers
{
    ...
    [Authorize]
    [AuthorizedAction]
    public class FeeHeadMasterController : ControllerBase
    {
        private readonly IFeeHeadService _service;
        public FeeHeadMasterController(IFeeHeadService service)
        {
            _service = service;
        }
        ...
        /// </summary>
        /// <returns></returns>      
        [HttpGet("{collegeId}/{feeHeadTypeMasterId}")]
        public async Task<IActionResult> GetAll(long collegeId, long feeHeadTypeMasterId)
        {
            if (collegeId < 1 && feeHeadTypeMasterId < 1)
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, null, string.Empty);
            var res = await _service.GetAll(collegeId, feeHeadTypeMasterId);
            if (res != null)
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(FeeHeadMasterVM feeHeadMasterVM)
        {
            if (feeHeadMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(feeHeadMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, FeeHeadMasterVM feeHeadMasterVM)
        {
            if (id != feeHeadMasterVM.Id || feeHeadMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(feeHeadMasterVM);
        ...
        /// <returns></returns>
        [HttpGet("{collegeId}/{feeHeadTypeMasterId}")]
        public async Task<IActionResult> GetOptions(long collegeId, long feeHeadTypeMasterId)
        {
            if (collegeId < 1  && feeHeadTypeMasterId < 1)
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, false, string.Empty);
            var res = await _service.GetOptions(collegeId,feeHeadTypeMasterId);
            if (res != null)
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.Ok, true, res, string.Empty);
            return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, res, string.Empty);
        }
