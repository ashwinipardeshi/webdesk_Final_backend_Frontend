...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CountryGMasterController : ControllerBase
    {
        private readonly ICountryService _service;
        public CountryGMasterController(ICountryService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Insert(CountryGMasterVM countryGmasterVM)
        {
            if (countryGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent,false);
            }
            var res = await _service.Insert(countryGmasterVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CountryGMasterVM countryGmasterVM)
        {
            if (id != countryGmasterVM.Id || countryGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent,false);
            }
            var res = await _service.Update(countryGmasterVM);
