...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class EmployeeTypeGMasterController : ControllerBase
    {
        private readonly IEmployeeTypeService _service;
        public EmployeeTypeGMasterController(IEmployeeTypeService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>        
        [HttpPost]  
        public async Task<IActionResult> Insert(EmployeeTypeGMasterVM employeeTypeGmasterVM)
        {
            if (employeeTypeGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(employeeTypeGmasterVM);
        ...
        /// <returns></returns>
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, EmployeeTypeGMasterVM employeeTypeGmasterVM)
        {
            if (id != employeeTypeGmasterVM.Id || employeeTypeGmasterVM == null)

            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(employeeTypeGmasterVM);
