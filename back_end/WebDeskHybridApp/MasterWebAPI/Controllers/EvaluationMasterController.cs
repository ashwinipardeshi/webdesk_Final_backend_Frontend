...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class EvaluationMasterController : ControllerBase
    {
        private readonly IEvaluationService _service;
        public EvaluationMasterController(IEvaluationService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(EvaluationMasterVM evaluationMasterVM)
        {
            if (evaluationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(evaluationMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, EvaluationMasterVM evaluationMasterVM)
        {
            if (id != evaluationMasterVM.Id || evaluationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(evaluationMasterVM);
