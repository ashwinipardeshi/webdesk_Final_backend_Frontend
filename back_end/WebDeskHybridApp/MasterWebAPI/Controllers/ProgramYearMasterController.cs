...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ProgramYearMasterController : ControllerBase
    {
        private readonly IProgramYearService _service;
        public ProgramYearMasterController(IProgramYearService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(ProgramYearMasterVM programYearMasterVM)
        {
            if (programYearMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(programYearMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ProgramYearMasterVM programYearMasterVM)
        {
            if (id != programYearMasterVM.Id || programYearMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(programYearMasterVM);
