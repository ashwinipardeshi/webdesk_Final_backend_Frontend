...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class TimeSlotGMasterController : ControllerBase
    {
        private readonly ITimeSlotService _service;
        public TimeSlotGMasterController(ITimeSlotService service)
        {
            _service = service;
        }
        ...
        /// Insert
        /// </summary>
        /// <param name="timeSlotGmasterVM"></param>
        /// <returns></returns>       
        [HttpPost] 
        public async Task<IActionResult> Insert(TimeSlotGMasterVM timeSlotGmasterVM)
        {
            if (timeSlotGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(timeSlotGmasterVM);
        ...
        /// <returns></returns>     
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, TimeSlotGMasterVM timeSlotGmasterVM)
        {
            if (id != timeSlotGmasterVM.Id || timeSlotGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(timeSlotGmasterVM);
