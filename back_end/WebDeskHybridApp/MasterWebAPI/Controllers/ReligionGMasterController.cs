...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ReligionGMasterController : ControllerBase
    {
        private readonly IReligionService _service;
        public ReligionGMasterController(IReligionService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(ReligionGMasterVM religionGMasterVM)
        {
            if (religionGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(religionGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ReligionGMasterVM religionGMasterVM)
        {
            if (id != religionGMasterVM.Id || religionGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(religionGMasterVM);
