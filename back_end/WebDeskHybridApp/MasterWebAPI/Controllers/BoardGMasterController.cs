...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class BoardGMasterController : ControllerBase
    {
        private readonly IBoardService _service;
        public BoardGMasterController(IBoardService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(BoardGMasterVM boardGMasterVM)
        {
            if (boardGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(boardGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, BoardGMasterVM boardGMasterVM)
        {
            if (id != boardGMasterVM.Id || boardGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(boardGMasterVM);
