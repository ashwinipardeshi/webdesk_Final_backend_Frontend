...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SemesterDetailsMasterController : ControllerBase
    {
        private readonly ISemesterDetailsService _service;
        public SemesterDetailsMasterController(ISemesterDetailsService service)
        {
            _service = service;
        }
        ...
        [HttpGet("{programMasterId}/{programYearId}/{academicYearId}")]
        public async Task<IActionResult> GetAll(long programMasterId, long programYearId, long academicYearId)
        {
            if (programMasterId < 1  && programYearId < 1 && academicYearId < 1)
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, null, string.Empty);
            var res = await _service.GetAll(programMasterId,programYearId,academicYearId);
            if (res != null)
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(SemesterDetailsMasterVM semesterDetailsMasterVM)
        {
            if (semesterDetailsMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(semesterDetailsMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, SemesterDetailsMasterVM semesterDetailsMasterVM)
        {
            if (id != semesterDetailsMasterVM.Id || semesterDetailsMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(semesterDetailsMasterVM);
