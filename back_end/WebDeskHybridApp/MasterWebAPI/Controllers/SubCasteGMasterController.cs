...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SubCasteGMasterController : ControllerBase
    {
        private readonly ISubCasteService _service;
        public SubCasteGMasterController(ISubCasteService service)
        { 
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(SubCasteGMasterVM subCasteGMasterVM)
        {
            if (subCasteGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(subCasteGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, SubCasteGMasterVM subCasteGMasterVM)
        {
            if (id != subCasteGMasterVM.Id || subCasteGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(subCasteGMasterVM);
