...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class AccreditationMasterController : ControllerBase
    {
        private readonly IAccreditationService _service;
        public AccreditationMasterController(IAccreditationService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(AccreditationMasterVM accreditationMasterVM)
        {
            if (accreditationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(accreditationMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, AccreditationMasterVM accreditationMasterVM)
        {
            if (id != accreditationMasterVM.Id || accreditationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(accreditationMasterVM);
