...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class StudyMasterController : ControllerBase
    {
        private readonly IStudyService _service;
        public StudyMasterController(IStudyService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(StudyMasterVM studyMasterVM)
        {
            if (studyMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(studyMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, StudyMasterVM studyMasterVM)
        {
            if (id != studyMasterVM.Id || studyMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(studyMasterVM);
