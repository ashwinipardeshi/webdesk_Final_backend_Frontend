...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CommonSubjectListController : ControllerBase
    {
        private readonly ICommonSubjectListService _service;
        public CommonSubjectListController(ICommonSubjectListService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(CommonSubjectListGMasterVM commonSubjectListGMasterVM)
        {
            if (commonSubjectListGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(commonSubjectListGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CommonSubjectListGMasterVM commonSubjectListGMasterVM)
        {
            if (id != commonSubjectListGMasterVM.Id || commonSubjectListGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(commonSubjectListGMasterVM);
