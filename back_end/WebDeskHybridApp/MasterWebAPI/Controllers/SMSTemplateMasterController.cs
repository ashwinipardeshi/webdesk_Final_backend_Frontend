...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SMSTemplateMasterController : ControllerBase
    {
        private readonly ISMSTemplateService _service;
        public SMSTemplateMasterController(ISMSTemplateService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(SMSTemplateMasterVM smsTemplateMasterVM)
        {
            if (smsTemplateMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(smsTemplateMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, SMSTemplateMasterVM smsTemplateMasterVM)
        {
            if (id != smsTemplateMasterVM.Id || smsTemplateMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(smsTemplateMasterVM);
