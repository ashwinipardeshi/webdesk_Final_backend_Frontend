...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CasteGMasterController : ControllerBase
    {
        private readonly ICasteService _service;
        public CasteGMasterController(ICasteService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(CasteGMasterVM casteGMasterVM)
        {
            if (casteGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(casteGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CasteGMasterVM casteGMasterVM)
        {
            if (id != casteGMasterVM.Id || casteGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(casteGMasterVM);
