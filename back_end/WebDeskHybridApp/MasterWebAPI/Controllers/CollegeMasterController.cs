...
namespace MasterWebAPI.Controllers
{
    ...
    public class CollegeMasterController : ControllerBase
    {
        ...
        public CollegeMasterController(ICollegeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(CollegeMasterVM collegeMasterVM)
        {
            if (collegeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent,false);
            }
            var res = await _service.Insert(collegeMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CollegeMasterVM collegeMasterVM)
        {
            if (id != collegeMasterVM.Id || collegeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(collegeMasterVM);
