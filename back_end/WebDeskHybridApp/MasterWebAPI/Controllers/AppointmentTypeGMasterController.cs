...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class AppointmentTypeGMasterController : ControllerBase
    {
        private readonly IAppointmentTypeService _service;
        public AppointmentTypeGMasterController(IAppointmentTypeService service)
        {
            _service = service;
        }
        ...
        /// <returns></returns>      
        [HttpPost] 
        public async Task<IActionResult> Insert(AppointmentTypeGMasterVM appointmentTypeGmasterVM)
        {
            if (appointmentTypeGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(appointmentTypeGmasterVM);
        ...
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, AppointmentTypeGMasterVM appointmentTypeGmasterVM)
        {
            if (id != appointmentTypeGmasterVM.Id || appointmentTypeGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(appointmentTypeGmasterVM);
