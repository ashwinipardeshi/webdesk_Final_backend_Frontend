...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    ...
    public class BloodGroupGMasterController : ControllerBase
    {
        private readonly IBloodGroupService _service;
        public BloodGroupGMasterController(IBloodGroupService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(BloodGroupGMasterVM bloodGroupGMasterVM)
        {
            if (bloodGroupGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(bloodGroupGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, BloodGroupGMasterVM bloodGroupGMasterVM)
        {
            if (id != bloodGroupGMasterVM.Id || bloodGroupGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent,false);
            }
            var res = await _service.Update(bloodGroupGMasterVM);
