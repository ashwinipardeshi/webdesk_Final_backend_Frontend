...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CasteCategoryGMasterController : ControllerBase
    {
        private readonly ICasteCategoryService _service;
        public CasteCategoryGMasterController(ICasteCategoryService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(CasteCategoryGMasterVM casteCategoryGMasterVM)
        {
            if (casteCategoryGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(casteCategoryGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, CasteCategoryGMasterVM casteCategoryGMasterVM)
        {
            if (id != casteCategoryGMasterVM.Id || casteCategoryGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(casteCategoryGMasterVM);
