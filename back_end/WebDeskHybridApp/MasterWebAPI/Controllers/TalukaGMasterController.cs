...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class TalukaGMasterController : ControllerBase
    { 
        private readonly ITalukaService _service;
        public TalukaGMasterController(ITalukaService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(TalukaGMasterVM talukaGmasterVM)
        {
            if (talukaGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(talukaGmasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, TalukaGMasterVM talukaGmasterVM)
        {
            if (id != talukaGmasterVM.Id || talukaGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(talukaGmasterVM);
