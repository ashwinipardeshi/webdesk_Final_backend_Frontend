...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class RelationGMasterController : ControllerBase
    {
        private readonly IRelationService _service;
        public RelationGMasterController(IRelationService service)
        {
            _service = service;
        }
        ...
        /// Insert
        /// </summary>
        /// <param name="relationGmasterVM"></param>
        /// <returns></returns>
       
        [HttpPost] 
        public async Task<IActionResult> Insert(RelationGMasterVM relationGmasterVM)
        {
            if (relationGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(relationGmasterVM);
        ...
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, RelationGMasterVM relationGmasterVM)
        {
            if (id != relationGmasterVM.Id || relationGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(relationGmasterVM);
