...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SMTPConfigMasterController : ControllerBase
    {
        private readonly ISMTPConfigService _service;
        public SMTPConfigMasterController(ISMTPConfigService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(SMTPConfigMasterVM smtpConfigMasterVM)
        {
            if (smtpConfigMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(smtpConfigMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, SMTPConfigMasterVM smtpConfigMasterVM)
        {
            if (id != smtpConfigMasterVM.Id || smtpConfigMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(smtpConfigMasterVM);
