...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class StreamMasterController : ControllerBase
    {
        private readonly IStreamService _service;
        public StreamMasterController(IStreamService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(StreamMasterVM streamMasterVM)
        {
            if (streamMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(streamMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, StreamMasterVM streamMasterVM)
        {
            if (id != streamMasterVM.Id || streamMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(streamMasterVM);
