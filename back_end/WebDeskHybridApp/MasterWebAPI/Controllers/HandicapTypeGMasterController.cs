...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class HandicapTypeGMasterController : ControllerBase
    {
        private readonly IHandicapTypeService _service;
        public HandicapTypeGMasterController(IHandicapTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(HandicapTypeGMasterVM handicapTypeGMasterVM)
        {
            if (handicapTypeGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(handicapTypeGMasterVM);
        ...
        /// <returns></returns>
         [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, HandicapTypeGMasterVM handicapTypeGMasterVM)
        {
            if (id != handicapTypeGMasterVM.Id || handicapTypeGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(handicapTypeGMasterVM);
