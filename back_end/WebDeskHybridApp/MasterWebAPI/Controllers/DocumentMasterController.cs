...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class DocumentMasterController : ControllerBase
    {
        ...
        public DocumentMasterController(IDocumentService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(DocumentMasterVM documentMasterVM)
        {
            if (documentMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(documentMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, DocumentMasterVM documentMasterVM)
        {
            if (id != documentMasterVM.Id || documentMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(documentMasterVM);
