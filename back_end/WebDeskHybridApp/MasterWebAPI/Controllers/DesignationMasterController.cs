...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class DesignationMasterController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(DesignationMasterVM designationMasterVM)
        {
            if (designationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(designationMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, DesignationMasterVM designationMasterVM)
        {
            if (id != designationMasterVM.Id || designationMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(designationMasterVM);
