...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class CourseCategoryGMasterController : ControllerBase
    {
        private readonly ICourseCategoryService _service;
        public CourseCategoryGMasterController(ICourseCategoryService service)
        {
            _service = service;
        }
        ...
        /// Insert
        /// </summary>
        /// <param name="courseCategoryGmasterVM"></param>
        /// <returns></returns>
       
        [HttpPost]
        public async Task<IActionResult> Insert(CourseCategoryGMasterVM courseCategoryGmasterVM)
        {
            if (courseCategoryGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(courseCategoryGmasterVM);
        ...
        [HttpPut("{id}")] 
        public async Task<IActionResult> Update(long id, CourseCategoryGMasterVM courseCategoryGmasterVM)
        {
            if (id != courseCategoryGmasterVM.Id || courseCategoryGmasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(courseCategoryGmasterVM);
