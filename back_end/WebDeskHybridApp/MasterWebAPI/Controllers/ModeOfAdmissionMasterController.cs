...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class ModeOfAdmissionMasterController : ControllerBase
    {
        private readonly IModeOfAdmissionMasterService _service;
        public ModeOfAdmissionMasterController(IModeOfAdmissionMasterService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(ModeOfAdmissionMasterVM modeOfAdmissionMasterVM)
        {
            if (modeOfAdmissionMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(modeOfAdmissionMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, ModeOfAdmissionMasterVM modeOfAdmissionMasterVM)
        {
            if (id != modeOfAdmissionMasterVM.Id || modeOfAdmissionMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(modeOfAdmissionMasterVM);
