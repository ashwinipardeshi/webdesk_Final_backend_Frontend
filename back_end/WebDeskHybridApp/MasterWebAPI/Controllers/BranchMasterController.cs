...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class BranchMasterController : ControllerBase
    {
        ...
        public BranchMasterController(IBranchService service)
        {
            _service = service;
        }
        ...
        [HttpGet("{collegeId}/{programMasterId}")]
        public async Task<IActionResult> GetAll(long collegeId, long programMasterId)
        {
            if (collegeId < 1 && programMasterId < 1 )
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, null, string.Empty);
            var res = await _service.GetAll(collegeId,programMasterId);
            if (res != null)
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(BranchMasterVM branchMasterVM)
        {
            if (branchMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(branchMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, BranchMasterVM branchMasterVM)
        {
            if (id != branchMasterVM.Id || branchMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(branchMasterVM);
        ...
        /// <returns></returns>
        [HttpGet("{collegeId}/{programMasterId}")]
        public async Task<IActionResult> GetOptions(long collegeId, long programMasterId)
        {
            if (collegeId < 1  && programMasterId < 1 )
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, null, string.Empty);
            var res = await _service.GetOptions(collegeId,programMasterId);
            if (res != null)
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.Ok, true, res, string.Empty);
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false, res, string.Empty);
        }
