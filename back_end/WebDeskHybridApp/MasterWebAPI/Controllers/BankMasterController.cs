...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class BankMasterController : ControllerBase
    {
        ...
        public BankMasterController(IBankService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(BankMasterVM bankMasterVM)
        {
            if (bankMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(bankMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, BankMasterVM bankMasterVM)
        {
            if (id != bankMasterVM.Id || bankMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(bankMasterVM);
