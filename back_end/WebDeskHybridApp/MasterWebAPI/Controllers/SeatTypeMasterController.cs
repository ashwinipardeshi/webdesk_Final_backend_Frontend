...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class SeatTypeMasterController : ControllerBase
    {
        private readonly ISeatTypeService _service;
        public SeatTypeMasterController(ISeatTypeService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(SeatTypeMasterVM seatTypeMasterVM)
        {
            if (seatTypeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(seatTypeMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, SeatTypeMasterVM seatTypeMasterVM)
        {
            if (id != seatTypeMasterVM.Id || seatTypeMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(seatTypeMasterVM);
