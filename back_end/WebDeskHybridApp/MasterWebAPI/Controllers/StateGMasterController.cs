...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class StateGMasterController : ControllerBase
    {
        private readonly IStateService _service;
        public StateGMasterController(IStateService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(StateGMasterVM stateGMasterVM)
        {
            if (stateGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(stateGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, StateGMasterVM stateGMasterVM)
        {
            if (id != stateGMasterVM.Id || stateGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Update(stateGMasterVM);
