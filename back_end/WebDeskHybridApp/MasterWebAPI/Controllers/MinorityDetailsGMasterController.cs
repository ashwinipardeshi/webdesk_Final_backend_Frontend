...
namespace MasterWebAPI.Controllers
{
   ...
   [AuthorizedAction] // RoleBasedMenus
    public class MinorityDetailsGMasterController : ControllerBase
    {
        private readonly IMinorityDetailsService _service;
        public MinorityDetailsGMasterController(IMinorityDetailsService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Insert(MinorityDetailsGMasterVM minorityDetailsGMasterVM)
        {
            if (minorityDetailsGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent, false);
            }
            var res = await _service.Insert(minorityDetailsGMasterVM);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, MinorityDetailsGMasterVM minorityDetailsGMasterVM)
        {
            if (id != minorityDetailsGMasterVM.Id || minorityDetailsGMasterVM == null)
            {
                return APIResponseFinal.Response((int)CommonCodes.APIResErrorCodes.NoContent , false);
            }
            var res = await _service.Update(minorityDetailsGMasterVM);
